...
from The_Vector.vec import Vec
...
def equal(A, B):
    ...
    for row, col in A.f:
        a_val = A.f[row, col]
        if a_val != 0:
            if (not (row, col) in B.f) or (B.f[row, col] != a_val):
                return False
    for row, col in B.f:
        b_val = B.f[row, col]
        if b_val != 0:
            if (not (row, col) in A.f) or (A.f[row, col] != b_val):
                return False
    return True
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, {(row, col): val*x for (row, col), val in M.f.items()})
...
def transpose(M):
    ...
    True
    """
    return Mat((set(M.D[1]), set(M.D[0])), {(col, row): val for (row, col), val in M.f.items()})
...
class Mat:
    ...
    def __str__(M, rows=None, cols=None):
        ...
        colw = {col: (1 + max([len(str(col))] + [
            len('{0:.{1}G}'.format(M[row, col], numdec)) if isinstance(M[row, col], int) or isinstance(M[row, col],
                                                                                                       float) else len(
                str(M[row, col])) for row in rows])) for col in cols}
        s1 = ' ' * (1 + pre + len(separator))
        s2 = ''.join(['{0:>{1}}'.format(str(c), colw[c]) for c in cols])
        s3 = ' ' * (pre + len(separator)) + '-' * (sum(list(colw.values())) + 1)
        s4 = ''.join(['{0:>{1}} {2}'.format(str(r), pre, separator) + ''.join(['{0:>{1}.{2}G}'.format(M[r, c], colw[c],

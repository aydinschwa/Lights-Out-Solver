...
def equal(u, v):
    ...
    u_keys = u.f.keys()
    v_keys = v.f.keys()
    for u_key, u_value in u.f.items():
        if u_value:
            if (u_key not in v_keys) or (u_value != v.f[u_key]):
                return False
    for v_key, v_value in v.f.items():
        if v_value:
            if (v_key not in u_keys) or (v_value != v.f[v_key]):
                return False
    return True
...
def add(u, v):
    ...
    """
    assert u.D == v.D
    return Vec(u.D, {key: u.f.get(key, 0) + v.f.get(key, 0) for key in list(v.f.keys()) + list(u.f.keys())})
...
def dot(u, v):
    ...
    """
    assert u.D == v.D
    return sum([u.f.get(key, 0) * v.f.get(key, 0) for key in u.D])
